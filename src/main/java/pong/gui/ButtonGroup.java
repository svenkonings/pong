...
public class ButtonGroup extends Group {
    ...
    public ProgressBar addBar(GuiBase gb) {
        ...
        bar.setPrefWidth(gb.getFieldWidth() * (1 - gb.getFieldToButtonWidth()) * 0.5 * 0.75);
        bar.setPrefHeight(gb.getFieldY() * 0.75);
        bar.setLayoutX(gb.getFieldX() + gb.getFieldWidth() - gb.getFieldWidth() * (1 - gb.getFieldToButtonWidth()) * 0.5 * 0.5 - bar.getPrefWidth() / 2);// + (bar.getMaxWidth()) / 2);//fieldX / 2);
